...
class WebhookTargetDetailView(DetailView):
    ...
    def get_object(self):
        if "pk" in self.kwargs:
            return get_object_or_404(WebhookTarget, pk=self.kwargs["pk"])
        if "identifier" in self.kwargs:
            return get_object_or_404(WebhookTarget, pk=self.kwargs["identifier"])
        raise Http404

    @cached_property
    def deliveries(self):
        return Delivery.objects.filter(webhoot_target=self.webhook_target)[
    ...
    def get_object(self):
        if "pk" in self.kwargs:
            return get_object_or_404(
                WebhookTarget, pk=self.kwargs["pk"], owner=self.request.user
            )
        if "identifier" in self.kwargs:
            return get_object_or_404(
                WebhookTarget, pk=self.kwargs["identifier"], owner=self.request.user
            )
        raise Http404

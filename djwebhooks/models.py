...
class WebhookTarget(TimeStampedModel):
    ...
    header_content_type = models.CharField(
        "Header content-type",
        max_length=255,
    ...
    def __str__(self):
        return "{}:{}:{}".format(self.event, self.target_url[:30], self.identifier)
...
class Delivery(TimeStampedModel):
    ...
    attempt = models.IntegerField(
        "How many times has this been attempted to be delivered"
    )
    hash_value = models.CharField(max_length=255, blank=True)

    notification = models.TextField(
        "Passed back from the Senderable object", blank=True
    )
    ...
    def __str__(self):
        return "{}=>{}=>{}".format(self.success, self.created, self.webhook_target)
